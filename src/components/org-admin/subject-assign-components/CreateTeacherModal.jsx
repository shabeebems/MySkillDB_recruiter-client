...
const CreateTeacherModal = ({
  ...
  const handleSubmit = async () => {
    if (!createTeacherFormData.name || !createTeacherFormData.email || !createTeacherFormData.mobile || !createTeacherFormData.departmentId) {
    ...
    try {
      ...
      const response = await postRequest(`/users`, {
        ...
        mobile: createTeacherFormData.mobile,
      ...
      if (response.data.success) {
        ...
        const errorMessage = response.data.message || 'Failed to create teacher';
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-xl w-full sm:max-w-md ring-1 ring-black/5 mt-auto sm:mt-0 mb-0 sm:mb-0 max-h-[calc(100vh-4rem)] sm:max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-neutral-200/50 px-5 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 z-10 rounded-t-3xl sm:rounded-t-2xl">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-lg sm:text-xl font-semibold text-neutral-900 tracking-tight flex items-center gap-2 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 bg-teal-50 rounded-xl flex items-center justify-center ring-1 ring-teal-200/50 flex-shrink-0">
                <i className="fas fa-chalkboard-teacher text-teal-600 text-xs sm:text-sm"></i>
              </div>
              <span className="truncate">Create New Teacher</span>
            </h3>
            <button 
              onClick={onClose} 
        ...
        <div className="p-5 sm:p-6 space-y-4 sm:space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 sm:py-3.5 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 bg-white text-neutral-900 placeholder-neutral-400 text-sm sm:text-base outline-none hover:border-neutral-300"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-11 pr-10 py-3 sm:py-3.5 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 bg-white text-neutral-900 text-sm sm:text-base font-medium outline-none appearance-none hover:border-neutral-300 cursor-pointer"
              >
        ...
        <div className="p-5 sm:p-6 border-t border-neutral-200 flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="flex-1 sm:flex-none px-5 py-3 bg-teal-600 hover:bg-teal-700 text-white font-medium rounded-xl transition-all duration-200 shadow-sm hover:shadow-md active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
            disabled={isLoading}
          >
            {isLoading ? (
