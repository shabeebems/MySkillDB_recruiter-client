          ...
          {generalError && (
            <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <button
                  onClick={() => handleFieldChange('_clearGeneralError', null)}
...
const FormSection = ({ editJobData, fieldErrors, departments, handleFieldChange }) => {
  ...
  <>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="relative" ref={companyWrapperRef}>
        ...
        value={editJobData.company}
          ...
          onFocus={() => {
              setShowCompanyDropdown(true);
              fetchCompanies(editJobData.company);
          }}
        ...
        {showCompanyDropdown && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                {isCompanyLoading ? (
                    ...
                    editJobData.company && <div className="p-2 text-center text-xs text-gray-500">No existing companies found. A new one will be created.</div>
                )}
            </div>
        )}
      </div>
    ...
    <DepartmentSelect 
      departments={departments}
      selectedDepartments={editJobData.departments || []}
...
const DepartmentSelect = ({ departments, selectedDepartments, onChange, error }) => {
            ...
            <>
              <div 
                className={`px-4 py-2.5 text-sm cursor-pointer flex items-center justify-between hover:bg-slate-50 transition-colors border-b border-slate-100 ${
                  departments.length > 0 && departments.length === selectedDepartments.length ? 'bg-blue-50 text-blue-700' : 'text-slate-900'
                }`}
                onClick={handleSelectAll}
              >
                <span className="font-semibold">Select All</span>
                {departments.length > 0 && departments.length === selectedDepartments.length && <i className="fas fa-check text-blue-600"></i>}
              </div>
              ...
              const isSelected = selectedDepartments.some(d => d._id === dept._id);
                ...
                <div 
                  key={dept._id}
                    className={`px-4 py-2.5 text-sm flex items-center justify-between transition-colors cursor-pointer hover:bg-slate-50 ${
...
const FormField = ({ label, type, value, onChange, placeholder, error, fieldName }) => (
  <div>
    <label className="block text-sm font-semibold text-slate-700 mb-2">
      {label}
    </label>
